...
class WatchOutSettings:
    ...
    def __init__(self, camera_password):
        ...
        if sys.platform == 'linux':
            ...
            self.ffmpeg_binary_path = self._user_path.joinpath('usr', 'bin', 'ffmpeg')
        ...
        # Format parameters: (ffmpeg_base_command, quality, resolution, grayscale, pix_fmt,
        # output_file(formatted with output format)
        self.ffmpeg_command_format_string = \
            "{ffmpeg_base_command:s} -frames:v 1 -q:v {quality:d} -vf scale={resolution:s}{grayscale:s} " + \
            "-pix_fmt {pix_fmt:s} -update 1 \"{output_file:s}\""
